...
void Player::buyAndPlace()
{
    ...
    } while (judgeForRefresh)//由于商店每回合自动刷新一次,因此判断放在买牌的后面
...
void Player::buyChess()
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        //会被释放吗
        Chess* storeChess = Chess::createById(myStore->idInStore[i]);
        //计算出备战席最近的空位
        int index = 0;
        for (; index < SEATS_NUM; index++) {
            if (mySeats[index] == nullptr)
                break;
        }

        //若已达人口上限
        //若当前卡槽的棋子买了后,不能合成,则不买
        if (meleeAttacks.size() + rangedAttacks.size() == myStore->level) {
        ...
        //过渡阶段:等级小于4
        //一费棋子只用来过渡,有二星就不用再买
        if (storeChess->originalCost == 1 && myStore->level <= 4) {
            bool isExist = 0;
    ...
    if (count == 2) {
        ...
        if (isToMerge) {
            if (c->isMelee) {
                ...
                meleeAttacks.erase(index[1]);              
                c->upgrade();
            }
            else {
                rangedAttacks.erase(index[0]);
                rangedAttacks.erase(index[1]);
                c->upgrade();
            }
            //对meleeAttacks操作,不知道有没有同步到棋盘和备战席
