...
void GridMap::swapChessOnGrids(HexCell* cella, HexCell* cellb)
{
    if (cella && cellb)
    {
        Chess* a = cella->chessInGrid;
        Chess* b = cellb->chessInGrid;
        cella->setChess(b);
        cellb->setChess(a);
    }
}
...
void GridMap::addChessToGrid(Chess* Inchess, HexCell* Incell)
{
    ...
    Inchess->initialPosition = Inchess->getPosition();
}
